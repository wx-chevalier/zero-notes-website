...
const getServiceName = data => {
	const name =
		data.metadata.serviceAbbreviation || data.metadata.serviceFullName;
...
exports.apis = () =>
	fsP
		...
		.then(files => {
			...
			const promises = Object.keys(result).map(x =>
				loadServiceMethods(x, result[x])
			);
